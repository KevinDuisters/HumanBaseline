...
#-----------------------------------------------------------------------------------------------------------
# Table of efficient frontier
panels <- c(rep("Metabolon",pvec[1]),rep("BMFL",pvec[2]+pvec[3]),rep("SomaLogic",pvec[4]))
samples <- c(rep("plasma",pvec[1]+pvec[2]),rep("urine",pvec[3]),rep("plasma",pvec[4]))
...
mysort <- c(which(K$panel=="SomaLogic"),which(K$panel=="Metabolon"),which(K$panel=="BMFL"))
...
Platform <- numeric(length(K$panel))
Platform[which(K$panel=="SomaLogic")] <- 1
Platform[which(K$panel=="Metabolon")] <- 2
Platform[which(K$panel=="BMFL")] <- 3
Platform <- Platform[mysort]
Platform <- factor(Platform,levels=c(1,2,3),labels=mylabs)

V <- data.frame(sigmab=s.b[mysort],sigmaeps=s.eps[mysort],sigmatau=s.tau[mysort],name=shortnames[mysort])


ggp <- (ggplot(V, aes(x=sigmaeps, y=sigmab, size=(sigmatau))) +
...
for(k in 1:4){
  ...
  ni <- ncol(sm)
  #print(histnames[k])
  #print(rbind(cbind(sm,rowSums(sm,na.rm=T)),colSums(cbind(sm,rowSums(sm,na.rm=T)),na.rm=T)))
  
  #signifnamelist[[k]]
  #tab <- table(signifcountlist[[k]])
  #midpoints <- barplot(tab,main=histnames[k],ylim=c(0,1.1*tab[1]))
  #text(x=midpoints,y=tab+0.05*tab[1],labels=tab)
  
  par(xpd=T)
  plot(0:(nrow(sm)),0:(ncol(sm)),type="n",axes=F,xlab="",ylab="",main=paste0(histnames[k]," (",pvec[k],")"))
  for(r in 1:(ni-1)){
    for(c in (r+1):(ni)){
      polygon(x=c(-1.5+c,-0.5+c,-0.5+c,-1.5+c,-1.5+c),y=c(-0.5+r,-0.5+r,0.5+r,0.5+r,-0.5+r),col=
                cols[sum( (0:10)/10 <= 2*sm[r,c]/pvec[k])])
    }}
  axis(side=1,colnames(sm),at=0:(ni-1),tick=F,cex.axis=1,line=-1,las=2)
  axis(side=2,colnames(sm),at=(1:ni),tick=F,las=1,cex.axis=1)
  
  # upper triangle numbers
  for(r in 1:ni){
    for(c in r:ni){
      if(c!=r){text(x=r-1,y=c,sm[c,r],cex=0.8)}
      if(c==r){text(x=r-1,y=c,"-",cex=0.8)}
    }
  }
