...
pdf("Figures/Contrasts.pdf",width=8.7)
cx <- 1.5

#par(mfrow=c(1,3),mar=c(6,8,6,2)+0.1)


layout(rbind(c(1,2,3),c(4,4,4)),heights=c(0.5,0.5))
for(k in 1:3){
  sm <- signiflist[[k]]
  ni <- ncol(sm)
  par(xpd=T)
  ...
  title(paste0(histnames[k]," [",pvec[k],"]"),cex.main=cx,line=3)
...
text(x=0.45,y=yc+0.15,"% significant biochemicals in panel",cex=cx)
